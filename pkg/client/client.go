...
// ResetConsumerGroupOffsets resets committed offsets for a consumer group.
// If topic is empty, all topics in the group are reset.
// strategy must be "earliest", "latest", or "timestamp".
// timestampMs is used only for the "timestamp" strategy (milliseconds since epoch).
func (client *Client) ResetConsumerGroupOffsets(
	group string,
	topic string,
	strategy string,
	timestampMs int64,
	...
	go func() {
		...
		case "to-datatime":
			spec = kafka.NewOffsetSpecForTimestamp(timestampMs)
		default:
		...
		for _, tps := range listResult.ConsumerGroupsTopicPartitions {
			for _, tp := range tps.Partitions {
				if topic == "" || *tp.Topic == topic {
					offsetsRequest[tp] = spec
				}
			}
		}

		if len(offsetsRequest) == 0 {
			errorChan <- fmt.Errorf("no committed partitions found for group '%s'", group)
			return
		}
		...
		targetOffsets, err := client.ListOffsets(ctx, offsetsRequest,
			kafka.SetAdminIsolationLevel(kafka.IsolationLevelReadCommitted))
		if err != nil {
			errorChan <- fmt.Errorf("failed to resolve target offsets: %w", err)
			return
		}

		var tpOffsets []kafka.TopicPartition
		for tp, info := range targetOffsets.ResultInfos {
			tpCopy := tp
			tpCopy.Offset = info.Offset
			tpOffsets = append(tpOffsets, tpCopy)
		}
